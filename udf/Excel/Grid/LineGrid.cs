...
public abstract record РядСетки(int Номер, IntPtr ЛистИд) : ИМожетИзменятьВидимость
{
        ...
        double? размер = null, bool авторазмер = false)
    {
        ...
        foreach (var лист in refПоЛисту)
            ИзменитьВидимость(new ExcelReference(лист), тип, размер, авторазмер);
    }

    protected XlCall.XlReturn ИзменитьВидимость(double? размер = null, bool авторазмер = false)
    {
        if (ВидимЛи() is true ^ размер is <= 0)
            return XlCall.XlReturn.XlReturnAbort;
        return ИзменитьВидимость(GetAsExcelReference(), GetType(), размер, авторазмер);
    }

    public XlCall.XlReturn Скрыть()
    {
        return Скрыть(GetAsExcelReference(), GetType());
    }
    ...
    {
        if (тип == typeof(ExcelСтрока))
            return XlCall.xlcRowHeight;
        if (тип == typeof(ExcelСтолбец))
            return XlCall.xlcColumnWidth;
