...
public sealed class АвтоРасчетПриостановить : IDisposable {
    ...
    public АвтоРасчетПриостановить() {
        ...
        if (_typeNum == НастройкиПересчета.Ручной) return;
    ...
    public void Dispose() { ВосстановитьСохраненныеЗначения(); }
    ...
    private XlCall.XlReturn ВосстановитьСохраненныеЗначения() {
        return !_измененЛи
                   ...
                   : XlCall.TryExcel(XlCall.xlcOptionsCalculation, out _,
                                     (int)_typeNum,
                                     _iter, _maxNum, _maxChange, _update, _precision, _date1904, _calcSave,
                                     _saveValues);
    }
}